...
const createCharAnimation = (scene: Phaser.Scene, charName: string) => {
    createAnimation(scene,charName, charName + '-look-down', 	charName + '-standing');
    createAnimation(scene,charName, charName + '-look-left',  	charName + '-standing-left');
    createAnimation(scene,charName, charName + '-look-right', 	charName + '-standing-right');
    createAnimation(scene,charName, charName + '-look-up', 	    charName + '-standing-up');
};

const createAnimation = (scene: Phaser.Scene, charname: string, key: string, pngName: string) =>{
    scene.anims.create({
        key: key,
        frames: scene.anims.generateFrameNames(charname, { start: 1, end: 2, prefix: pngName, suffix:'.png'}),
        repeat: -1,
        frameRate: 4
    });
};
